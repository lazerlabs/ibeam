...
from ibeam.src.two_fa_handlers.external_request_handler import (
    ExternalRequestTwoFaHandler,
)
from ibeam.src.two_fa_handlers.google_msg_handler import GoogleMessagesTwoFaHandler
from ibeam.src.two_fa_handlers.http_callback import HttpCallbackTwoFaHandler
...
def select(
    ...
    elif handler_name == "HTTP_CALLBACK":
        handler = HttpCallbackTwoFaHandler(outputs_dir=outputs_dir)
    elif handler_name == "CUSTOM_HANDLER":
        handler = load_custom_two_fa_handler(custom_two_fa_handler, inputs_dir)(
...
def load_custom_two_fa_handler(two_fa_handler_fqp, inputs_dir: os.PathLike):
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            module_name, os.fspath(handler_filepath)
        )
        ...
        if str(e) == f"No module named '{module_name}'":
            raise RuntimeError(
                f'Invalid handler path: "{two_fa_handler_fqp}". Module "{module_name}" not found. You need to provide the MODULE_NAME.CLASS_NAME of your custom handler as IBEAM_CUSTOM_TWO_FA_HANDLER environment variable.'
            ) from e
        else:
            raise e
    ...
    try:
        ...
        if str(e) == f"module '{module_name}' has no attribute '{class_name}'":
            raise RuntimeError(
                f'Invalid handler path: "{two_fa_handler_fqp}". Module "{module_name}" has no class "{class_name}". You need to provide the MODULE_NAME.CLASS_NAME of your custom handler as IBEAM_CUSTOM_TWO_FA_HANDLER environment variable.'
            ) from e
        else:
            raise e
